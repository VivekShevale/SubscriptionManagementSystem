...
def generate_subscription_number() -> str:
    ...
    Queries the highest existing number and increments.
    """
    from app.models import Subscription
    last = Subscription.query.order_by(Subscription.id.desc()).first()
    if last and last.subscription_number:
        try:
            num = int(last.subscription_number[1:]) + 1
...
def compute_next_invoice_date(start_date: date, billing_period: str, count: int = 1) -> date:
    ...
    if billing_period == "daily":
        ...
        # Add months safely
        month = start_date.month - 1 + count
        year = start_date.year + month // 12
        month = month % 12 + 1
        day = min(start_date.day, [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][month - 1])
        return date(year, month, day)
    elif billing_period == "yearly":
        return date(start_date.year + count, start_date.month, start_date.day)
